(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".ourhouse__slider")) new core(".ourhouse__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            watchOverflow: true,
            navigation: {
                prevEl: ".ourhouse__prev",
                nextEl: ".ourhouse__next"
            },
            pagination: {
                el: ".ourhouse__pagination",
                clickable: true
            },
            keyboard: {
                enabled: true,
        ...
        if (document.querySelector(".ipoteka__slider")) new core(".ipoteka__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
            ...
            watchOverflow: false,
            navigation: {
                prevEl: ".ipoteka__prev",
                nextEl: ".ipoteka__next"
            },
            pagination: {
                el: ".ipoteka__pagination",
                clickable: true
            },
            scrollbar: {
                el: ".ipoteka__scrollbar",
                draggable: true
            },
            keyboard: {
                enabled: true,
        ...
        if (document.querySelector(".projectmax__slider")) var projectMaxSlider = new core(".projectmax__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (document.querySelector(".projectmin__slider")) var projectMinSlider = new core(".projectmin__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
        ...
        if (projectMaxSlider) {
            projectMaxSlider.on("slideChange", (function() {
                var slideIndex = projectMaxSlider.realIndex;
                projectMinSlider.slideTo(slideIndex);
            }));
            projectMinSlider.on("slideChange", (function() {
                var slideIndex = projectMinSlider.realIndex;
                projectMaxSlider.slideTo(slideIndex);
            }));
        }
    }
    ...
    let tabTitles = document.querySelectorAll(".tabs1projects__title");
    if (tabTitles.length > 0) tabTitles[0].classList.add("_tab-active");
    let navigationBlocks = document.querySelectorAll(".tabs2projects__navigation");
    navigationBlocks.forEach((function(block) {
        let firstChild = block.firstElementChild;
        if (null !== firstChild) firstChild.classList.add("_tab-active");
    }));
    ...
    ratings.forEach((function(rating) {
        var numberOfStars = parseInt(rating.getAttribute("data-quan"));
        var originalContent = rating.innerHTML;
        for (var i = 1; i < numberOfStars; i++) rating.innerHTML += originalContent;
    }));
    var equipmentItems = document.querySelectorAll(".project_equipment_it");
    if (equipmentItems) equipmentItems.forEach((function(item) {
        item.addEventListener("click", (function() {
            item.classList.toggle("active");
            equipmentItems.forEach((function(otherItem) {
                if (otherItem !== item) otherItem.classList.remove("active");
            }));
        }));
    }));
    let ipotekaTitles = document.querySelectorAll(".ipoteka_title ");
    if (ipotekaTitles) for (let index = 0; index < ipotekaTitles.length; index++) {
        const element = ipotekaTitles[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let ipotekaTexts = document.querySelectorAll(".ipoteka_text ");
    if (ipotekaTexts) for (let index = 0; index < ipotekaTexts.length; index++) {
        const element = ipotekaTexts[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let mainPageTexts = document.querySelectorAll(".main_page_text ");
    if (mainPageTexts) for (let index = 0; index < mainPageTexts.length; index++) {
        const element = mainPageTexts[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let mainPageTtitles = document.querySelectorAll(".main_page_title ");
    if (mainPageTtitles) for (let index = 0; index < mainPageTtitles.length; index++) {
        const element = mainPageTtitles[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let homeToptitles = document.querySelectorAll(".hometop_title ");
    if (homeToptitles) for (let index = 0; index < homeToptitles.length; index++) {
        const element = homeToptitles[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let homeTopTexts = document.querySelectorAll(".hometop_text ");
    if (homeTopTexts) for (let index = 0; index < homeTopTexts.length; index++) {
        const element = homeTopTexts[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let linksFunds = document.querySelectorAll(".type_list ul li a");
    if (linksFunds) for (let index = 0; index < linksFunds.length; index++) {
        const element = linksFunds[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
    let fundamentTexts = document.querySelectorAll(".fundament_subtext");
    if (fundamentTexts) for (let index = 0; index < fundamentTexts.length; index++) {
        const element = fundamentTexts[index];
        let newElement = element.innerText.replace(/\s/g, "").length;
        if (!newElement) element.remove();
    }
